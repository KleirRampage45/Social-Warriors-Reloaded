...
class Game {
    ...
    _renderUnitShop() {
        ...
        for (const [id, unit] of config.units) {
            if (unit.type !== 'u') continue;

            const cat = unit.category || 'Other';
            if (!categories[cat]) {
                categories[cat] = [];
            }
            categories[cat].push({ id, ...unit });
        }
        ...
        for (const [catName, items] of Object.entries(categories)) {
            ...
            for (const item of items.slice(0, 20)) {
                ...
                html += `
                    <div class="shop-item ${canAfford ? '' : 'disabled'}" data-unit-id="${item.id}">
                        <div class="shop-item-icon">${item.name?.substring(0, 6) || item.id}</div>
                        <div class="shop-item-info">
                            <div class="shop-item-name">${item.name || item.id}</div>
                            <div class="shop-item-cost">${costStr}</div>
        ...
        if (html === '') {
            html = '<div class="shop-category"><div class="category-name">No units available</div></div>';
        }
        ...
        listContainer.querySelectorAll('.shop-item:not(.disabled)').forEach(el => {
            el.addEventListener('click', () => {
                ...
                this._selectUnit(unitId);
            });
        });
    }

    _selectUnit(unitId) {
        const unit = this.config.units.get(unitId);
        if (!unit) return;
        ...
        console.log(`[Game] Selected unit: ${unit.name} (${unitId})`);
    }

    _loadQuestData() {
        this.playerData.quests = this.playerData.quests || {};
    }
    ...
    _renderQuestPanel() {
        ...
        const questFiles = [
            'villages/quest/100000001.json',
            'villages/quest/100000002.json',
            'villages/quest/100000003.json',
            'villages/quest/100000004.json',
            'villages/quest/100000005.json',
            'villages/quest/100000006.json',
        ];

        let html = '<div class="shop-category"><div class="category-name">Available Quests</div>';

        for (const questFile of questFiles) {
            const questId = questFile.split('/').pop().replace('.json', '');
            const completed = this.playerData.quests?.[questId]?.completed;

            html += `
                <div class="shop-item ${completed ? 'disabled' : ''}" data-quest-id="${questId}">
                    <div class="shop-item-icon">Q</div>
                    <div class="shop-item-info">
                        <div class="shop-item-name">Quest ${questId}</div>
                        <div class="shop-item-cost">${completed ? 'Completed' : 'Available'}</div>
                    </div>
                </div>
            `;
        }
        ...
        listContainer.querySelectorAll('.shop-item:not(.disabled)').forEach(el => {
            el.addEventListener('click', () => {
                ...
                this._selectQuest(questId);
            });
        });
    }

    _selectQuest(questId) {
        console.log(`[Game] Selected quest: ${questId}`);
        this.eventBus.emit('quest:selected', { questId });
    }
